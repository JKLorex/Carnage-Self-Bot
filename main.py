...
Alucard = commands.Bot(command_prefix=commands.when_mentioned_or(
    ">", "x", "$", "-", "a", "^", "!"), description='CARNAGE', intents=intents)
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                                ...
                                {Fore.CYAN}info: {Fore.GREEN}Developer discord server: https://discord.gg/wT8N3BWRJb
...
[ {ctx.prefix}Fun ] - команды фана.
[ {ctx.prefix}Reaction ] - команды пыток.
[ {ctx.prefix}Nsfw ] - NSFW команды.
[ {ctx.prefix}Mod ] - команды выдачи пизды.
[ {ctx.prefix}Crash ] - команды ебли сервера.
[ {ctx.prefix}Text ] - команды сообщений.
[ {ctx.prefix}Token ] - команды защиты аккаунта.
[ {ctx.prefix}Tools ] - никому ненужная хуйня.
[ {ctx.prefix}Animals ] - команды с феминистками.
[ {ctx.prefix}Status ] - команды крутышки.
[ {ctx.prefix}Games ] - элитный бравл старс.
-----------------------
[ Версия разраба ]
```
